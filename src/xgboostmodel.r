...
orderp <- read.csv("order_products__prior.csv", header = T)
ordert <- read.csv("order_products__train.csv", header = T)
...
data <- data %>% inner_join(prd,by="product_id") %>% inner_join(users,by="user_id")
head(data)
str(data)
...
subtrain <- train %>% sample_frac(0.8)
summary(subtrain)
